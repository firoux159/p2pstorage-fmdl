...
@Slf4j
public class P2PPTStoreService extends PushingDHTStoreService<BigInteger, NettyConnectionInfo, String, FileDTO> {
	
    private UniformRandomGenerator uniformRandomGenerator;
    private Random random;
    private int randomCount = 2; // default to select 2 nodes
    
    private ArrayList<NodeServer> nodes = new ArrayList<NodeServer>();
    
	
	public P2PPTStoreService(DHTKademliaNodeAPI<BigInteger, NettyConnectionInfo, String, FileDTO> dhtKademliaNode, ExecutorService executorService) {
	...
	public P2PPTStoreService(DHTKademliaNodeAPI<BigInteger, NettyConnectionInfo, String, FileDTO> dhtKademliaNode, ExecutorService executorService, int randomCount) {
    	this(dhtKademliaNode, executorService);
    	this.randomCount = randomCount;
    	
    	log.debug("[P2PPTStoreService] random count="+randomCount);
    }
    ...
    public KademliaMessage<BigInteger, NettyConnectionInfo, ?> handle(
    	...
    	log.debug("StoreService: P2PPTStoreService");
    	...
    	if (message.isAlive()){
    		...
    		if (!P2PMessageType.CLIENT_STORE_REQ.equals(message.getType())) {
    			...
    			log.debug("[P2PPTStoreService.handle] Routing table of node "+this.dhtKademliaNode.getId()+" updated with node="+message.getNode().getId());
    		}
        }
...
//            this.dhtKademliaNode.completeStore(end-start);
    ...
    protected EmptyKademliaMessage<BigInteger, NettyConnectionInfo> handleStoreRequest(
    	...
    	log.debug("[P2PPTStoreService.handleStoreRequest] 1");
    	
		DHTStoreKademliaMessage.DHTData<BigInteger, NettyConnectionInfo, String, FileDTO> data = message.getData();
		
		log.debug("[P2PStoreService.handleStoreRequest] 1.1 node="+message.getNode().getId()+", requester="+data.getRequester().getId()+", key="+data.getKey()+", value="+data.getValue().getName());
...
//            this.dhtKademliaNode.completeStore(end-start);
    	...
    	log.debug("[P2PPTStoreService.handleStoreRequest] 2");
    ...
    protected StoreAnswer<BigInteger, NettyConnectionInfo, String> handleStore(
    	...
    	// this request can either be to store actual data OR to store data mapping
    	// check the content of the "value" to be stored
    	log.debug("[handleStore 1] requester="+requester.getId()+", key="+key+", value="+value.getName()+", ext="+value.getExternalNode());
    	
//    	String dataMapping = value.getExternalNode();
//    	if (dataMapping != null) {
//    		// the request is to store data mapping
//    		return handleStoreMapping(caller, requester, key, value);
//    	}
...
//        ExternalNode<BigInteger, NettyConnectionInfo> externalNode2 = (ExternalNode<BigInteger, NettyConnectionInfo>)findNodeAnswer2.getNodes().get(0);
		...
		// randomly selects <randomCount> server
    	//ArrayList<NodeServer> randomNodes = new ArrayList<>(randomCount);
    	HashMap<NodeServer, Double> randomNodes = new HashMap<>(randomCount);
    	
    	int d = 0;
    	int size = nodes.size();
    	
    	log.debug("[handleStore 2] random count ="+randomCount+", nodes="+size);
    	
    	while (d < randomCount) {
    		int randomIndex = random.nextInt(size);
    		NodeServer node = nodes.get(randomIndex);
    		
    		log.debug("[handleStore 3] randomIndex ="+randomIndex+", random node="+node.getId()+", "+node.getName());
    		...
    		if (!randomNodes.containsKey(node)) {
    	    	...
    	    	log.debug("[handleStore 4] get workload status for node ="+node.getId());
    	        ...
    	        log.debug("[handleStore 5] node="+node.getId()+", statusResponse="+workloadResponse.toString());
    	...
    	HashMap<NodeServer, Double> sortedRandomNodes = sortByValue(randomNodes, 1);
    	
    	// select the first entry from the list (node with lowest workload)
...
//        this.dhtKademliaNode.completeFind(end-start);
    ...
    protected StoreAnswer<BigInteger, NettyConnectionInfo, String> storeDataToExternal (
    	...
    	log.debug("[P2PPTStoreService.storeDataToExterna 1l]");
    ...
    protected EmptyKademliaMessage<BigInteger, NettyConnectionInfo> handleStoreExternalRequest(
    	...
    	log.debug("[P2PPTStoreService.handleStoreExternalRequest 1]");
...
//        this.dhtKademliaNode.initiateStore();
    	...
    	this.handlerExecutorService.submit(() -> {
            ...
            if (storeAnswer.getResult().equals(StoreAnswer.Result.STORED)) {
            	...
            	log.debug("[P2PPTStoreService.handleStoreExternalRequest 2]");
...
//            this.dhtKademliaNode.completeStore(end-start);
    	...
    	log.debug("[P2PPTStoreService.handleStoreExternalRequest] 3");
...
//    protected EmptyKademliaMessage<BigInteger, NettyConnectionInfo> handleStoreExternalRequest(
//    		DHTStoreExternalKademliaMessage<BigInteger, NettyConnectionInfo, String, FileDTO> dhtStoreKademliaMessage) {
//
//    	log.debug("[P2PPTStoreService.handleStoreExternalRequest] 1");
//    	
//		DHTStoreExternalKademliaMessage.DHTData<BigInteger, NettyConnectionInfo, String, FileDTO> data = dhtStoreKademliaMessage.getData();
//		
//		log.debug("[P2PPTStoreService.handleStoreExternalRequest] data.getRequester() = "+data.getRequester().getId());
//		
//        StoreAnswer<BigInteger, NettyConnectionInfo, String> storeAnswer = handleStoreExternal(this.dhtKademliaNode, data.getRequester(), data.getKey(), data.getValue());
//        if (storeAnswer.getResult().equals(StoreAnswer.Result.STORED)) {
//            this.dhtKademliaNode.getMessageSender().sendAsyncMessage(
//                    this.dhtKademliaNode,
//                    data.getRequester(),
...
//    protected DHTStoreResultKademliaMessage<BigInteger, NettyConnectionInfo, String> handleStoreExternalRequest(
//    		DHTStoreKademliaMessage<BigInteger, NettyConnectionInfo, String, FileDTO> message) {
//
//    	
//    	log.debug("[P2PPTStoreService.handleStoreExternalRequest] 1");
...
//		log.debug("[P2PPTStoreService.handleStoreExternalRequest] data.getRequester() = "+data.getRequester().getId());
//		
//		log.debug("[P2PPTStoreService.handleStoreExternalRequest] 2, key="+data.getKey()+", value="+data.getValue());
//		
//        StoreAnswer<BigInteger, NettyConnectionInfo, String> storeAnswer = handleStoreExternal(this.dhtKademliaNode, data.getRequester(), data.getKey(), data.getValue());
//        
//        if (storeAnswer.getResult().equals(StoreAnswer.Result.STORED)) {
//
//        	log.debug("[P2PPTStoreService].handleStoreExternalRequest 3 " + storeAnswer.getNode().getId()+", "+storeAnswer.getResult());
//        	
//        	response = new DHTStoreResultKademliaMessage<>(
//        			new DHTStoreResultKademliaMessage.DHTStoreResult<>(data.getKey(), StoreAnswer.Result.STORED));
...
//        
//    	response = new DHTStoreResultKademliaMessage<>(
//    			new DHTStoreResultKademliaMessage.DHTStoreResult<>(data.getKey(), StoreAnswer.Result.FAILED));
//    	response.setNode(storeAnswer.getNode());
//    	
//    	return response;
//
//    }
    ...
    protected StoreAnswer<BigInteger, NettyConnectionInfo, String> handleStoreExternal(
    	...
    	log.debug("[P2PPTStoreService.handleStoreExternal 1] caller="+caller.getId()+", requester="+requester.getId());
    	...
    	log.debug("[P2PPTStoreService.handleStoreExternal 2]");
    	
		// STEP 1" store the first chunk of data into this node (actual node)
    	FileDTO chunk = chunks.get(0);
    	
    	String chunkKey = ""+(chunk.getName().hashCode() & 0x7fffffff);
    	
    	//String chunkKey = ""+chunk.getName().hashCode();
    	
    	log.debug("[P2PPTStoreService.handleStoreExternal 3] chunk = "+chunk.getName()+", "+chunkKey);
...
//		// STEP 2: store the mapping of first chunk to the 'supposedly storing node' (mapping node)
...
//		chunk.setExternalNode(actualNodeConnectionInfo);
//		
//    	BigInteger hash = this.dhtKademliaNode.getKeyHashGenerator().generateHash(chunkKey);
//    	
//        start = System.currentTimeMillis();
//        this.dhtKademliaNode.initiateFind();
//
...
//        this.dhtKademliaNode.completeFind(end-start);
//        
//        // this may return PASSED
//        storeAnswer = storeDataToClosestNode(this.dhtKademliaNode, requester, findNodeAnswer.getNodes(), chunkKey, chunk);
//		// TO-DO: handle exception i.e. if the store is failed
		...
		for (int i=1; i<chunks.size(); i++) {
			...
			//chunkKey = ""+chunk.getName().hashCode();
...
//	        NodeStatusMessage<BigInteger, NettyConnectionInfo> statusMessage = new NodeStatusMessage<>();
	        NodeWorkloadMessage<BigInteger, NettyConnectionInfo> workloadMessage = new NodeWorkloadMessage<>(new NodeWorkloadMessage.NodeWorkload<>(requester));
...
//	        NodeStatusResultMessage.NodeStatusResult<Double> statusData = (NodeStatusResultMessage.NodeStatusResult<Double>) statusResponse.getData();
//	        double node1Workload = statusData.getWorkload();
	        ...
	        NodeWorkloadResultMessage.NodeWorkloadResult<Double> workloadData = (NodeWorkloadResultMessage.NodeWorkloadResult<Double>) workloadResponse.getData();
	        double node1Workload = workloadData.getWorkload();
	        
	        long node1latency = System.currentTimeMillis() - start;
	        //double node1Magnitude = calculateMagnitude(node1Workload, node1latency);
	        //double node1Magnitude = node1Workload;
	        double node1Magnitude = workloadData.getWorkload();
	        
	        log.debug("[P2PPTStoreService.handleStoreExternal] 4: node1=" + externalNode1.getId() +", workload="+ node1Workload+", latency=" + node1latency+", magnitude=" + node1Magnitude);

	        start = System.currentTimeMillis();

//	        statusResponse = this.dhtKademliaNode.getMessageSender().sendMessage(
//	        		this.dhtKademliaNode,
//	        		externalNode2, 
//	        		statusMessage);
//	        
//	        statusData = (NodeStatusResultMessage.NodeStatusResult<Double>) statusResponse.getData();
//	        double node2Workload = statusData.getWorkload();
	        
	        workloadResponse = this.dhtKademliaNode.getMessageSender().sendMessage(
	        		this.dhtKademliaNode,
	        		externalNode2, 
	        		workloadMessage);

	        workloadData = (NodeWorkloadResultMessage.NodeWorkloadResult<Double>) workloadResponse.getData();
	        double node2Workload = workloadData.getWorkload();
	        
	        long node2latency = System.currentTimeMillis() - start;
	        //double node2Magnitude = calculateMagnitude(node2Workload, node2latency);
	        double node2Magnitude = node2Workload;

	        log.debug("[P2PPTStoreService.handleStoreExternal] 5: node2=" + externalNode2.getId() +", workload="+ node2Workload+", latency=" + node2latency+", magnitude=" + node2Magnitude);

	        if (node1Magnitude < node2Magnitude) { 
	        	// store at selected node (actual node)
		...
		log.debug("[P2PPTStoreService.handleStoreExternal] 6");
...
//            }
	...
	private double calculateMagnitude(double a, double b) {
	...
	// function to sort hashmap by values - ascending
	// copied from: https://www.geeksforgeeks.org/sorting-a-hashmap-according-to-values/
